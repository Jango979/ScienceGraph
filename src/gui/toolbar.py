...
class Toolbar(ctk.CTkFrame):
    def __init__(self, master, on_add_image, on_add_text, on_add_latex,
                 on_toggle_spacing, on_copy_style, on_paste_style, on_export, **kwargs):
        super().__init__(master, width=100, **kwargs)
        self._spacing_btn: ctk.CTkButton | None = None
        self._build(on_add_image, on_add_text, on_add_latex,
                    on_toggle_spacing, on_copy_style, on_paste_style, on_export)
    ...
    def _btn(self, text: str, cmd, color=None, ret=False) -> ctk.CTkButton | None:
        kw = {"text": text, "command": cmd, "width": 88, "height": 34}
        if color:
            kw["fg_color"] = color
